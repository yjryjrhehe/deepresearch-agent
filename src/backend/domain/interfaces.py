...
class SearchRepository(ABC):
    ...
    由 infrastructure/search/opensearch_repo.py 实现。
    """
...
class ReportRepository(ABC):
    ...
    @abstractmethod
    def update(self, report_id: str, report: Report, error_message: Optional[str] = None):
        """更新报告状态"""
        pass
...
class IngestionService(ABC):
    """
    文档摄入服务 (业务流程编排)。
    由 services/ingestion_service.py 实现。
    """
    @abstractmethod
    def process_document(self, source: DocumentSource):
        """
        编排完整的文档摄入流程（流程图1）。
        1. 调用 DocumentParser
        2. 调用 TextSplitter
        3. 循环调用 LLmClient.generate_chunk_enhancements
...
class RetrievalService(ABC):
    ...
    """
    @abstractmethod
    def retrieve_contexts(self, query: str) -> List[RetrievedChunk]:
        """
        编排完整的RAG检索流程（流程图2）。
        1. 调用 ILlmClient.rewrite_query
        2. 并发/循环调用 ISearchRepository.hybrid_search
        3. 并发/循环调用 ILlmClient.rerank
        4. 聚合、去重、返回最终的排序列表
        """
        pass
