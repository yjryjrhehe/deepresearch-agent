# DeepResearch-Agent 🚀

[![Python Version](https://img.shields.io/badge/Python-3.13+-blue.svg)](https://python.org)
[![FastAPI](https://img.shields.io/badge/FastAPI-Latest-green.svg)](https://fastapi.tiangolo.com)
[![React](https://img.shields.io/badge/React-19-61dafb.svg)](https://reactjs.org)
[![LangGraph](https://img.shields.io/badge/LangGraph-Latest-purple.svg)](https://langchain-ai.github.io/langgraph)
[![OpenSearch](https://img.shields.io/badge/OpenSearch-3-FEC042.svg)](https://opensearch.org)
[![License](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

## 📖 项目简介

DeepResearch-Agent 是一个基于 **多智能体协作** 的 **AI深度研究系统**，采用Clean Architecture架构设计，集成了文档解析、RAG（检索增强生成）、多模型LLM支持等企业级功能。

该系统能够：
- **智能解析**各类复杂文档（PDF、Word等）
- **自动化**生成研究计划和深度分析
- **实时交互式**的智能体协作流程
- **可视化**的进度追踪和结果展示
- **企业级**的可观测性和追踪能力

---

## ✨ 核心特性

### 🎯 多智能体协作系统
- **协调器（Orchestrator）**: 全局流程控制与任务调度
- **规划器（Planner）**: 智能生成研究任务大纲
- **执行器（Worker）**: 深度研究与内容生成
- **反思器（Reflector）**: 质量评估与结果优化

### 📚 智能文档解析
- **Docling引擎**: 支持复杂文档结构解析（表格、公式、图片）
- **VLM增强**: 视觉语言模型增强文档理解
- **多模态处理**: 自动识别公式、图表、表格等复杂元素
- **智能分块**: 基于语义和结构的文档切片优化

### 🔍 高级检索系统
- **OpenSearch**: 高性能混合检索引擎
- **BGE Reranker**: GPU加速的文档重排序
- **多模型支持**: 支持多种嵌入模型和重排序模型
- **语义检索**: 结合向量化与关键词的混合检索

### 💬 实时交互
- **SSE流式传输**: 实时展示研究进度
- **人机协作**: 支持人工审核和反馈迭代
- **进度可视化**: 直观展示各阶段执行状态
- **Markdown渲染**: 支持LaTeX数学公式的富文本展示

### 🔧 可观测性
- **Langfuse集成**: 全链路追踪和性能监控
- **结构化日志**: 分级日志记录系统
- **指标追踪**: 智能体性能与成本分析

---

## 🏗️ 架构设计

### 技术栈

#### 后端
- **FastAPI**: 高性能异步Web框架
- **LangGraph**: 智能体编排框架
- **Docling**: 文档解析引擎
- **OpenSearch**: 搜索引擎与向量存储
- **LiteLLM**: 多模型统一代理
- **Pydantic**: 数据验证与序列化

#### 前端
- **React 19**: 现代化前端框架
- **Vite**: 极速构建工具
- **Tailwind CSS**: 原子化CSS框架
- **React-Markdown**: Markdown渲染
- **KaTeX**: 数学公式渲染

#### 基础设施
- **Docker Compose**: 容器化编排
- **Python 3.13+**: 现代Python特性
- **UV**: 高性能包管理器

### 系统架构图

```
┌─────────────────────────────────────────────────────────────┐
│                        Frontend (React)                      │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │ 研究任务启动  │  │  实时进度展示  │  │  结果查看     │      │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘      │
└─────────┼──────────────────┼──────────────────┼──────────────┘
          │                  │                  │
          ▼                  ▼                  ▼
┌─────────────────────────────────────────────────────────────┐
│                      Backend API (FastAPI)                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │  /api/research/* │  │ /api/ingest/*  │  │ /api/document/*  │      │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘      │
└─────────┼──────────────────┼──────────────────┼──────────────┘
          │                  │                  │
          ▼                  ▼                  ▼
┌─────────────────────────────────────────────────────────────┐
│                Multi-Agent Layer (LangGraph)                │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │协调器    │  │ 规划器   │  │ 执行器   │  │ 反思器   │   │
│  └─────┬────┘  └─────┬────┘  └─────┬────┘  └─────┬────┘   │
│        │            │            │            │          │
│        └────────────┴────────────┴────────────┘          │
└─────────────────────┬─────────────────────────────────────┘
                      │
                      ▼
┌─────────────────────────────────────────────────────────────┐
│                    Infrastructure Layer                     │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │ OpenSearch   │  │  TEI Reranker│  │  Documents   │      │
│  │ (Search + KV)│  │ (GPU Cache)  │  │  Storage     │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
└─────────────────────────────────────────────────────────────┘
```

---

## 🚀 快速开始

### 环境要求

- **Python**: 3.13+
- **Node.js**: 18+
- **Docker**: 20.10+
- **GPU**: 可选（用于BGE重排序加速）

### 1. 克隆项目

```bash
git clone https://github.com/your-org/deepresearch-agent.git
cd deepresearch-agent
```

### 2. 配置环境变量

```bash
# 复制环境变量模板
cp .env.example .env

# 编辑配置文件，填入你的API密钥
# - OpenAI API Key
# - Anthropic API Key
# - 其他云服务API密钥
```

### 3. 启动基础设施服务

```bash
# 启动OpenSearch和依赖服务
docker-compose up -d opensearch-node opensearch-dashboards tei-reranker
```

### 4. 配置LiteLLM代理

```bash
# 安装依赖
uv sync

# 启动LiteLLM代理（端口4000）
litellm --config config.yaml
```

### 5. 启动后端服务

```bash
# 启动FastAPI服务
uvicorn src.backend.api.server:app --host 0.0.0.0 --port 8000 --reload
```

### 6. 启动前端应用

```bash
cd src/frontend

# 安装依赖
npm install

# 启动开发服务器
npm run dev
```

访问 `http://localhost:5173` 开始使用！

---

## 📋 配置说明

### LiteLLM模型配置

在 `config.yaml` 中配置你的LLM模型：

```yaml
model_list:
  - model_name: "glm-4-flashx"  # 智谱GLM模型
    litellm_params:
      model: "openai/glm-4-flashx-250414"
      api_base: "https://open.bigmodel.cn/api/paas/v4/"
      api_key: "YOUR_API_KEY"

  - model_name: "qwen3-max"     # 阿里通义千问
    litellm_params:
      model: "openai/qwen3-max"
      api_base: "https://dashscope.aliyuncs.com/compatible-mode/v1"
      api_key: "YOUR_API_KEY"

  # 本地Ollama模型
  - model_name: "qwen3-4b-local"
    litellm_params:
      model: "ollama/qwen3:4b"
      api_base: "http://localhost:11434"
```

### 环境变量配置

在 `.env` 文件中配置关键参数：

```env
# ====================
# LLM配置
# ====================
RESEARCH_LLM_MODEL="qwen3-max"      # 研究主模型
DOCLING_VLM_MODEL="glm-4v-plus"     # 文档解析VLM
EMBEDDING_LLM_MODEL="qwen3-embedding-4b-local"

# ====================
# OpenSearch配置
# ====================
OPENSEARCH_HOST="localhost"
OPENSEARCH_PORT=9200
OPENSEARCH_INDEX_NAME="rag_system_chunks_async"

# ====================
# Langfuse追踪
# ====================
LANGFUSE_SECRET_KEY="sk-..."
LANGFUSE_PUBLIC_KEY="pk-..."
LANGFUSE_BASE_URL="http://localhost:3000"

# ====================
# TEI重排序器
# ====================
TEI_RERANK_BASE_URL="http://localhost:8082"
```

---

## 📖 使用指南

### 1. 上传并解析文档

点击"上传文档"按钮，选择PDF或Word文档。系统将：
- 自动解析文档结构
- 提取文本、表格、公式
- 智能分块并生成向量索引

### 2. 创建研究任务

在输入框中描述你的研究目标：
- 例如："分析当前AI大模型的发展趋势"
- 例如："研究量子计算在金融领域的应用"

### 3. 智能体协作流程

系统将自动执行以下流程：

1. **规划阶段**
   - 规划器生成详细研究大纲
   - 展示给用户进行审核

2. **执行阶段**
   - 执行器基于检索的文档内容进行深度研究
   - 实时流式输出研究进展

3. **反思阶段**
   - 反思器评估研究结果质量
   - 必要时自动迭代优化

4. **报告生成**
   - 汇总所有研究结果
   - 生成结构化的研究报告

### 4. 实时进度监控

所有阶段进度通过SSE实时传输到前端：
- 📊 可视化进度条
- 📝 实时日志输出
- 🔄 支持中断和恢复

---

## 🔌 API文档

### 研究任务接口

#### 启动研究
```http
POST /api/research/start
Content-Type: application/json

{
  "thread_id": "unique-thread-id",
  "goal": "your research question"
}
```

#### 实时流式输出
```http
GET /api/research/stream/{thread_id}?goal=your_question
Accept: text/event-stream
```

#### 审核研究计划
```http
POST /api/research/review
Content-Type: application/json

{
  "thread_id": "unique-thread-id",
  "action": "approve" | "revise",
  "feedback": "optional feedback"
}
```

### 文档上传接口

#### 上传文档
```http
POST /api/ingest/upload
Content-Type: multipart/form-data

file: [document file]
```

---

## 🛠️ 项目结构

```
deepresearch-agent/
├── src/
│   ├── backend/
│   │   ├── api/                    # API层
│   │   │   ├── server.py          # FastAPI入口
│   │   │   └── schemas.py         # 数据模型
│   │   ├── core/                  # 核心层
│   │   │   ├── config.py          # 配置管理
│   │   │   └── logging.py         # 日志配置
│   │   ├── domain/                # 领域层
│   │   │   ├── models.py          # 业务模型
│   │   │   └── interfaces.py      # 接口定义
│   │   ├── infrastructure/        # 基础设施层
│   │   │   ├── agents/            # 多智能体系统
│   │   │   ├── parse/             # 文档解析
│   │   │   ├── repository/        # 数据存储和检索
│   │   │   ├── llm/               # LLM集成
│   │   │   └── langfuse/          # 监控追踪
│   │   └── services/              # 服务层
│   │       ├── agent_service.py   # 深度研究智能体服务
│   │       ├── ingestion_service.py # 数据摄入和解析服务
│   │       └── factory.py
│   └── frontend/
│       ├── src/
│       │   ├── main.jsx           # 应用入口
│       │   ├── App.jsx            # 主组件
│       │   ├── index.css          # 全局样式
│       │   └── App.css            # 组件样式
│       ├── package.json
│       └── vite.config.js
├── models/                        # TEI模型文件
│   └── BAAI/bge-reranker-base
|—— langfuse/                      # langfuse部署文件
|	└── .env					 # 环境变量
|	└── docker-compose.yml         # 容器编排
├── uploads/                       # 上传文件
├── docker-compose.yml             # 容器编排
├── config.yaml                    # LiteLLM配置
├── pyproject.toml                 # Python依赖
├── .env                           # 环境变量
└── README.md                      # 项目文档
```

---

## 🧪 开发指南

### 添加新的LLM模型

1. 在 `config.yaml` 中添加模型配置
2. 在 `src/backend/llm/factory.py` 中注册模型
3. 更新 `.env` 文件中的模型名称

### 自定义智能体提示词

提示词模板位于 `src/backend/infrastructure/agents/prompt/`：
- `planner_prompt.py` - 规划器提示词
- `worker_prompt.py` - 执行器提示词
- `reflector_prompt.py` - 反思器提示词
- `reporter_prompt.py` - 报告生成提示词

### 扩展文档解析器

1. 在 `src/backend/infrastructure/parse/` 下创建新的解析器
2. 实现 `Ingestor` 接口
3. 在 `factory.py` 中注册解析器

---

## 🚢 部署

### 生产环境部署

#### 1. 使用Docker Compose

```bash
# 构建并启动所有服务
docker-compose -f docker-compose.yml -f docker-compose.prod.yml up -d
```

#### 2. 使用Kubernetes

项目支持Kubernetes部署，详见 `k8s/` 目录下的部署清单。

#### 3. 云原生部署

支持部署到：
- **AWS** (ECS/EKS)
- **Azure** (Container Instances/AKS)
- **GCP** (Cloud Run/GKE)

---

## 📊 性能优化

### 系统调优建议

1. **GPU配置**
   - 使用RTX 4090/3090加速BGE重排序
   - 配置CUDA内存池优化

2. **OpenSearch优化**
   - 调整JVM堆内存：`OPENSEARCH_JAVA_OPTS=-Xms2g -Xmx2g`
   - 配置SSD存储提升IO性能
   - 开启索引压缩

3. **模型缓存**
   - 配置Redis缓存LLM响应
   - 使用TEI Reranker的GPU内存缓存

---

## 🤝 贡献指南

我们欢迎所有形式的贡献！

### 提交PR流程

1. Fork本仓库
2. 创建特性分支：`git checkout -b feature/amazing-feature`
3. 提交更改：`git commit -m 'feat: add amazing feature'`
4. 推送分支：`git push origin feature/amazing-feature`
5. 提交Pull Request

### 报告Bug

使用GitHub Issues报告Bug，请包含：
- 详细的复现步骤
- 系统环境信息
- 错误日志
- 期望的行为

### 提出Feature Request

我们期待听到您的想法！请在Issue中详细描述：
- 功能目标
- 使用场景
- 预期效果

---

## 📈 路线图

### v1.1 (规划中)
- [ ] 支持更多文档格式 (Excel, PPT, Markdown)
- [ ] 集成Web搜索能力
- [ ] 多语言文档支持
- [ ] 协作研究报告分享

### v1.2 (规划中)
- [ ] 可视化研究流程编辑器
- [ ] 自定义智能体提示词UI
- [ ] 研究模板系统
- [ ] 批量研究任务

### v2.0 (展望)
- [ ] 多模态智能体（图像、视频）
- [ ] 实时协作编辑
- [ ] 知识图谱可视化
- [ ] 企业级SSO集成

---

## 📄 许可证

本项目基于MIT许可证开源 - 查看 [LICENSE](LICENSE) 文件了解详情

---

## 🙏 致谢

感谢以下开源项目：
- [LangGraph](https://langchain-ai.github.io/langgraph) - 智能体编排框架
- [Docling](https://github.com/DS4SD/docling) - 文档解析引擎
- [OpenSearch](https://opensearch.org) - 搜索引擎
- [FastAPI](https://fastapi.tiangolo.com) - Web框架
- [React](https://reactjs.org) - 前端框架

---

## 📞 联系我们

- 🐛 问题反馈：[GitHub Issues](https://github.com/your-org/deepresearch-agent/issues)
- 💬 社区讨论：[GitHub Discussions](https://github.com/your-org/deepresearch-agent/discussions)
- 📧 邮箱：deepresearch@example.com
- 🐦 Twitter：[@DeepresearchAI](https://twitter.com/DeepresearchAI)

---

<div align="center">
  <b>让AI为您的研究加速 🚀</b>
  <br>
  <i>如果这个项目对您有帮助，请给我们一个 ⭐</i>
</div>
